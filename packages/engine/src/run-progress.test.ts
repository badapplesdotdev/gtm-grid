...
beforeEach(() => {
  dir = mkdtempSync(join(tmpdir(), "run-progress-test-"));
...
describe("runColumn onCell progress stream", () => {
  it("emits running then done for each processed cell, with the final value", async () => {
    ...
    expect(events.every((e) => e.columnId === upper.id)).toBe(true);

    const byRow = (rowId: string) => events.filter((e) => e.rowId === rowId);
    for (const [rowId, value] of [[r1.id, "ADA"], [r2.id, "GRACE"]] as const) {
  ...
  it("emits a terminal error event for a failing cell", async () => {
    const table = db.createTable("Boom");
  ...
  it("does not abort the run when the onCell callback throws", async () => {
    const table = db.createTable("Resilient");
    const col = db.createColumn({
      tableId: table.id,
      name: "Out",
      kind: "function",
    ...
    expect(db.getCell(row.id, col.id)?.value).toBe("ok");
