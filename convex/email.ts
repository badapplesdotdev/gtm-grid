 ...
 *
 * Flows that send mail: account email VERIFICATION + PASSWORD RESET (Convex
 * Auth's `Password` provider via the OTP providers in convex/auth.ts) and
 * workspace INVITES (convex/invitations.ts). They all funnel through
 * {@link sendEmail} so there is one place that talks to Resend and one set of
 * templates.
 *
...
import { ICON_COLOR_B64, ICON_WHITE_B64 } from "./emailAssets.js";
